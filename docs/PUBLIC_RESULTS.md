...
공개 저장소에 들어가는 실제 결과는 **결과 유효성 검토가 끝난 canonical result**를 기준으로 합니다. 입력·실행·파싱 문제로 무효가 된 run이나 아직 원인이 해결되지 않은 결과를 공개 baseline 결과로 확정하지 않습니다.
...
예상 밖 결과나 실행 이상이 있었다면 공개 결과 파일을 확정하기 전에 입력 전달, 실행 조건, 파싱, 시나리오 명확성, 공개 기대값과 평가 기준을 검토합니다.

- 무효 실행이면 최종 baseline 집계에서 제외합니다.
- 벤치마크 결함이면 시나리오 버전을 갱신하고 다시 평가합니다.
- 충분히 판단되지 않으면 미해결 상태로 두고 공개 canonical result로 확정하지 않습니다.
- 유효성 검토를 통과한 실행 중 유리한 결과만 임의로 선택하지 않습니다.
...
반복 집계에는 같은 조건으로 인정되는 **유효 실행**만 사용합니다.
